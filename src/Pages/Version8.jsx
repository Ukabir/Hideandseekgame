...
const GameCanvas_v8 = ({ initialWidth = 800, initialHeight = 600 }) => {
  ...
  // --- MAIN LOOP (modified seeker logic) ---
  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    ...
    const update = () => {
      if (!found) {
        ...
        if (inCone) {
          ...
          // if obstacle ahead, gently curve around
          if (willCollide(seeker.x, seeker.y, seeker.angle)) {
            seeker.angle += (Math.random() > 0.5 ? 1 : -1) * Math.PI / 6;
          }
          ...
          // patrol smarter: random but obstacle-aware
          patrolTimer--;
          if (patrolTimer <= 0) {
          ...
          if (
            ...
            seeker.angle += Math.PI / 2; // turn sideways instead of reversing
