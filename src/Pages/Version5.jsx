...
const GameCanvas_v5 = ({ width, height }) => {
  const canvasRef = useRef(null);
  const keysRef = useRef({});
  const hiderRef = useRef({ x: 100, y: 100 });
  const seekerRef = useRef({ x: 400, y: 300 });
  const [found, setFound] = useState(false);

  const speed = 2.5; // hider speed
  const seekerSpeed = 1.6; // seeker speed
  const radius = 20;
  ...
  useEffect(() => {
    ...
    const update = () => {
      if (!found) {
        ...
        const dirX = x - seeker.x;
        const dirY = y - seeker.y;
        const distance = Math.sqrt(dirX * dirX + dirY * dirY);

        if (distance > 0) {
          seeker.x += (dirX / distance) * seekerSpeed;
          seeker.y += (dirY / distance) * seekerSpeed;
        }
